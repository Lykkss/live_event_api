...
CMD ["sh", "-c", "uv run python manage.py migrate --noinput && uv run python manage.py collectstatic --noinput && uv run gunicorn mspr2_api.wsgi:application --bind 0.0.0.0:8655 --workers 4"]